SELECT disciplina_id, sigla
  ...
  INNER JOIN (SELECT disciplina_id disciplina_id_com_5 FROM (
    SELECT disciplina_id, COUNT(disciplina_id) num_resp_com_5
      FROM (SELECT DISTINCT pergunta_id, disciplina_id
        FROM ipdw_respostas
        WHERE resposta  = 5 AND
          semestre_id = 21)
    GROUP BY disciplina_id)
    WHERE num_resp_com_5 = (SELECT COUNT(pergunta_id)
